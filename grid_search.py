...
def main(args: argparse.Namespace) -> float:
    ...
    model = sklearn.model_selection.GridSearchCV(pipe, {"poly__degree": [1,2],
    ...
    pipeline = sklearn.pipeline.Pipeline([['MinMax', sklearn.preprocessing.MinMaxScaler()],
                                         ['poly', sklearn.preprocessing.PolynomialFeatures(degree=best['poly__degree'])],
                                        ['logistic', sklearn.linear_model.LogisticRegression(random_state=args.seed, C=best['logistic__C'], solver=best['logistic__solver'])]])
